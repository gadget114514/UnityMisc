...
namespace Twonek {
	public class RUKHANKAShaderReplacer : EditorWindow
	{

		//		public GameObject target;
		public static string shaderName;
		
		[MenuItem("Tools/RUKHANKA ShaderReplacer by 2nek")]
		static void Init()
		{
			RUKHANKAShaderReplacer window = (RUKHANKAShaderReplacer )EditorWindow.GetWindow(typeof(RUKHANKAShaderReplacer ));
			window.Show();
		...
		void OnGUI()
		{
			...
			shaderName = "Shader Graphs/AnimatedLitShader HDRP";
			...
			if (GUILayout.Button("Replace")) {
				...
				Debug.Log("Replace Shaders=" + orgShader.name + " -> " + shader.name);
				if (shader.name == "") {
